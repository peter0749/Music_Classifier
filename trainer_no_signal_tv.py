...
def total_variation_loss(x):
    assert K.ndim(x) == 3 ## (batch#, sample#, 1)
    err = K.square(x[:, :total_samp - 1, :] - x[:, 1:, :])
    return K.sum(K.pow(err, 1.25))
...
x = np.random.randn(1, total_samp, 1) * 1e-2 ## generate initial wav sound
