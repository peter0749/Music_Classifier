...
def deprocess_wav(x):
    return invert_spectrogram(x).flatten().clip(-1., 1.)
...
if not K.is_keras_tensor(input_tensor):
    input_layer = Input(tensor=input_tensor, shape=(img_nrows, img_ncols, 1))
else:
    input_layer = input_tensor

## VGG19 Net:
# Block 1
x = Conv2D(64, (3, 3), activation='relu', padding='same', name='block1_conv1', kernel_initializer='random_normal')(input_layer)
...
pretrained_vgg19 = vgg19.VGG19(input_tensor=None,
                    weights='imagenet', include_top=False)

vgg19_dict = dict([(layer.name, layer) for layer in pretrained_vgg19.layers])
model_dict = dict([(layer.name, layer) for layer in model.layers])

## load weights from vgg19 as initial weights
for vgg19_lname in vgg19_dict:
    for model_lname in model_dict:
        if vgg19_lname==model_lname:
            try:
                model_dict[model_lname].set_weights(
                    vgg19_dict[vgg19_lname].get_weights()
                )
            except:
                continue
